'use strict';
var exchangeData = {}, exch = require('./lib/exchange'),
...
function submitRandomOrder(index) {
	...
	db.insertOne('transactions', ord, function(err, order) {
		if (exchangeData.trades && exchangeData.trades.length > 0) {
			...
			db.insert('transactions', trades, function(err, trade) {
				pauseThenTrade();
			});
		} else pauseThenTrade();
	});
...
app.configure(function() {
	app.use(express.bodyParser()); //req.body.user;req.body.password;etc
