...
module.exports = {
	...
	generateRandomOrder: function(exchangeData) {
		...
		var ran = Math.random();
		if (!buyExists && !sellExists) order.price = Math.floor(ran * priceRange) + priceFloor;
	...
	authenticate: function(username, password, callback) {
		db.findOne('users', {
			...
			if (user && (user.password == encrypPassword(password))) callback(err, user._id);
	...
	addStock: function(uid, stock, callback) {
		...
		db.push('users', new ObjectID(uid), {
			portfolio: stock
		}, doCallback);
	},
...
function encrypPassword(plainText) {
