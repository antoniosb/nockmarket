...
var db = new Db('js_node_market', new Server(host, port, {}), {
	native_parser: false
});
...
module.exports = {
	...
	insert: function(name, item, callback) {
		db.collection(name).insert(item, callback);
	},
	insertOne: function(name, item, callback) {
		module.exports.insert(name, item, function(err, itens) {
			callback(err, itens[0]);
		});
	},
